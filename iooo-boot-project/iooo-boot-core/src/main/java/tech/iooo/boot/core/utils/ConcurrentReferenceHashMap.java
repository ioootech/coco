...
/**
 * A {@link ConcurrentHashMap} that uses {@link ReferenceType#SOFT soft} or {@linkplain ReferenceType#WEAK weak} references for both {@code keys} and {@code values}.
 *
 * <p>This class can be used as an alternative to
 * {@code Collections.synchronizedMap(new WeakHashMap<K, Reference<V>>())} in order to support better performance when accessed concurrently. This implementation follows the same
 * design constraints as {@link ConcurrentHashMap} with the exception that {@code null} values and {@code null} keys are supported.
 *
 * <p><b>NOTE:</b> The use of references means that there is no guarantee that items
 * placed into the map will be subsequently available. The garbage collector may discard references at any time, so it may appear that an unknown thread is silently removing
 * entries.
 *
...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
   ...
   *
   * @param initialCapacity the initial capacity of the map
   * @param loadFactor the load factor. When the average number of references per table exceeds this value resize will be attempted
   */
  ...
  /**
   * Get the hash for a given object, apply an additional hash function to reduce collisions. This implementation uses the same Wang/Jenkins algorithm as {@link ConcurrentHashMap}.
   * Subclasses can override to provide alternative hashing.
   *
  ...
  /**
   * Remove any entries that have been garbage collected and are no longer referenced. Under normal circumstances garbage collected entries are automatically purged as items are
   * added or removed from the Map. This method can be used to force a purge, and is useful when the Map is read frequently but updated less often.
   */
  ...
  /**
   * A reference to an {@link Entry} contained in the map. Implementations are usually wrappers around specific Java reference implementations (e.g., {@link SoftReference}).
   */
  ...
  protected final class Segment extends ReentrantLock {
    ...
    /**
     * Array of references indexed using the low order bits from the hash. This property should only be set along with {@code resizeThreshold}.
     */
    ...
    /**
     * The threshold when resizing of the references should occur. When {@code count} exceeds this value references will be resized.
     */
    ...
    /**
     * Restructure the underlying data structure when it becomes necessary. This method can increase the size of the references table as well as purge any references that have been
     * garbage collected.
     *
  ...
  /**
   * Strategy class used to manage {@link Reference}s. This class can be overridden if alternative reference types need to be supported.
   */
  protected class ReferenceManager {
    ...
    /**
     * Return any reference that has been garbage collected and can be purged from the underlying structure or {@code null} if no references need purging. This method must be
     * thread safe and ideally should not block when returning {@code null}. References should be returned once and only once.
     *
