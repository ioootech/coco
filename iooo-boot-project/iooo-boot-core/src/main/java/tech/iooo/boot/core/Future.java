...
public interface Future<T> extends AsyncResult<T>, Handler<AsyncResult<T>> {
  ...
  /**
   * Bridges this future to a {@link CompletionStage} instance.
   * <p>
   ...
   * <p>
   * The future handling methods will be called from the thread that completes {@code completionStage}.
   *
   * @param completionStage a completion stage
   * @param <T> the result type
   * @return a future that resolves when {@code completionStage} resolves
   */
